...
- `PostgresBinPath` — **только для PostgreSQL**, необязательное. Каталог с `pg_dump`/`pg_basebackup`/`psql`/`pg_ctl` — override авто-резолва. По умолчанию агент сам ищет клиент под мажорную версию сервера (реестр Windows + стандартные каталоги установки → `PATH`). Задавайте поле только при нестандартной установке или когда нужно жёстко закрепить конкретную версию. Подробнее — в [postgres.md](postgres.md).
...
- `StorageName` — ссылается на запись в `Storages[]` по имени. Должно указывать на S3-хранилище — SFTP не поддерживается.
...
Расписание — общее с БД (дефолтный cron агента), либо override по имени file-set через дашборд.

**Работает только с S3.** При указании SFTP-хранилища бэкап завершится с ошибкой и статусом `failed` в дашборде.
