...
public sealed class SftpUploadProviderIntegrationTests
{
    private SftpUploadProvider _provider = null!;
    private SftpSettings _settings = null!;
    ...
    public void OneTimeSetUp()
    {
        ...
        var prefix = IntegrationConfig.MakeUniquePrefix(nameof(SftpUploadProviderIntegrationTests));
        _settings = new SftpSettings
        {
            Host = settings.Host,
            Port = settings.Port,
        ...
        _provider = new SftpUploadProvider(_settings, NullLogger<SftpUploadProvider>.Instance);
    ...
    [Test]
    public async Task UploadBytes_WithWrongPassword_ThrowsAuthError()
    {
        var bad = new SftpSettings
        {
            Host = _settings.Host,
            Port = _settings.Port,
        ...
        await using var badProvider = new SftpUploadProvider(bad, NullLogger<SftpUploadProvider>.Instance);

        try
        {
            await badProvider.UploadBytesAsync([1], "auth-check", _cts.Token);
            Assert.Fail("Expected an authentication-related exception.");
        }
        catch (Exception ex) when (ex is UnauthorizedAccessException or IOException
                                   or Renci.SshNet.Common.SshAuthenticationException
                                   or Renci.SshNet.Common.SshConnectionException)
        {
            Assert.Pass($"Got expected auth failure: {ex.GetType().Name}");
        }
    }
