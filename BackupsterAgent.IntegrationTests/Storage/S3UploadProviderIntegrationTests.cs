...
public sealed class S3UploadProviderIntegrationTests
{
    private S3UploadProvider _provider = null!;
    ...
    {
        Assume.That(
            IntegrationConfig.TryGetS3Settings(out var baseSettings),
            Is.True,
            "S3:* not configured; set via dotnet user-secrets or BACKUPSTER_INTEGRATION_S3__* env vars.");

        _bucket = IntegrationConfig.MakeBucketName();
        _settings = new S3Settings
        {
            EndpointUrl = baseSettings.EndpointUrl,
            AccessKey = baseSettings.AccessKey,
            SecretKey = baseSettings.SecretKey,
            Region = baseSettings.Region,
            BucketName = _bucket,
        };

        _cts = new CancellationTokenSource(TimeSpan.FromMinutes(2));

        await IntegrationConfig.EnsureBucketAsync(_settings, _bucket, _cts.Token);

        _provider = new S3UploadProvider(_settings, NullLogger<S3UploadProvider>.Instance);
    }
    ...
    public async Task OneTimeTearDown()
    {
        ...
        try
        {
            if (_settings is not null && _bucket is not null)
                await IntegrationConfig.DeleteBucketRecursiveAsync(_settings, _bucket, CancellationToken.None);
        }
        catch (Exception ex)
        {
            TestContext.Progress.WriteLine($"S3 cleanup failed: {ex.Message}");
        }
        _cts?.Dispose();
    }
    ...
    [Test]
    public async Task ListAsync_RecursivelyReturnsAllUnderPrefix()
    {
    ...
    [Test]
    public async Task UploadBytes_LongKey_StoresAndReadsBack()
    {
    ...
    [Test]
    public void UploadBytes_WithWrongSecret_ThrowsAmazonS3Exception()
    {
        var bad = new S3Settings
        {
            EndpointUrl = _settings.EndpointUrl,
            AccessKey = _settings.AccessKey,
            SecretKey = "wrong-on-purpose",
            Region = _settings.Region,
            BucketName = _settings.BucketName,
        };
        using var badProvider = new S3UploadProvider(bad, NullLogger<S3UploadProvider>.Instance);
