...
- `BackupRecordClient`, `ScheduleService`, `ConnectionSyncService`, `RetentionClient` — **20 с** на вызов.
...
| `startedAt`      | datetime (UTC)? | Опционально. Момент реального старта бэкапа по часам агента (ISO 8601). Если не передан — сервер использует `DateTime.UtcNow`. Агент отправляет это поле всегда; replay-сценарий (см. раздел про outbox в CLAUDE.md) передаёт реальное время старта бэкапа, случившегося до того, как дашборд стал доступен, чтобы `StartedAt` в UI не сдвигался на момент replay'я |
...
- **Клиент:** `Services/Dashboard/AgentTaskClient.FetchTaskAsync`
- **Тело запроса:** пустое
...
| Поле           | Тип    | Описание                                                                               |
|----------------|--------|----------------------------------------------------------------------------------------|
| `databaseName` | string | Имя БД из `Databases[].Database` — агент находит `DatabaseConfig` и запускает `BackupJob` |
...
- **Клиент:** `Services/Dashboard/AgentTaskClient.ReportProgressAsync`
- **Retry:** нет. Таймаут 3 с
...
- **Клиент:** `Services/Dashboard/AgentTaskClient.PatchTaskAsync`
- **Retry:** Polly, 3 попытки (1/2/4 с)
...
## 9. Retention — забрать просроченные записи
...
- **Клиент:** `Services/Dashboard/RetentionClient.GetExpiredAsync`
- **Retry:** нет
- **Тело запроса:** пустое
...
- **Клиент:** `Services/Dashboard/RetentionClient.DeleteAsync`
- **Retry:** Polly, 3 попытки (1/2/4 с)
- **Тело запроса:** пустое
...
## 11. Retention — пометить запись как «нет хранилища»
...
- **Клиент:** `Services/Dashboard/RetentionClient.MarkStorageUnreachableAsync`
- **Retry:** Polly, 3 попытки (1/2/4 с)
...
| `PATCH /task/{id}`              | `Workers/AgentTaskPollingService.CombineResults` → `AgentTaskClient.PatchTaskAsync` |
| `GET /backup-records/expired`   | —                                                                                       |
| `DELETE /backup-records/{id}`   | —                                                                                       |
| `POST /backup-records/mark-unreachable` | `Services/Backup/RetentionSweepService` (собирает список нерезолвнутых) → `RetentionClient.MarkStorageUnreachableAsync` |
