...
- **Retry:** нет (агент будет продолжать тикать)
- **Тело запроса:** пустое
...
- **Клиент:** `Services/Dashboard/RestoreTaskClient.PollAsync`
- **Тело запроса:** пустое
...
| `targetFileRoot`       | string? | Куда класть файлы. `null` = по оригинальным путям из манифеста                                     |
...
- **Клиент:** `Services/Dashboard/RestoreTaskClient.FinalizeAsync`
- **Retry:** Polly, 3 попытки (1/2/4 с)
...
| `POST /backup-record/{id}/progress` | `Services/Common/ProgressReporter.cs` → `BackupRecordClient.ReportProgressAsync` |
| `PATCH /backup-record/{id}`     | `Services/Backup/BackupJob.FinalizeAsync`                    |
| `GET /schedule`                 | —                                                            |
| `POST /connections`             | `Services/Dashboard/ConnectionSyncService.BuildPayload`      |
| `GET /restore-task`             | —                                                            |
| `POST /restore-task/{id}/progress` | `Services/Common/ProgressReporter.cs` → `RestoreTaskClient.ReportProgressAsync` |
| `PATCH /restore-task/{id}`      | `Workers/RestoreTaskPollingService.CombineResults` + `RestoreTaskClient.FinalizeAsync` |
