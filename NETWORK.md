...
- `.sql.gz.enc` — PostgreSQL / MySQL (`pg_dump` / `mysqldump` → gzip → encrypt).
...
Все расписания — индивидуальные per-(БД, режим). На одну БД может быть до двух записей (`logical` + `physical`), для file-set — одна запись (обычно с `backupMode: "logical"`).
...
| `overrides[].backupMode` | enum (string)              | `"logical"` или `"physical"`. Вместе с `databaseName` образует ключ записи |
...
Четвёртый топологический sync: агент сообщает дашборду свой список хранилищ. Отправляется один раз на старте, после успеха — останавливается. Внешний backoff 10 с → 5 мин до первого успеха.

- **Метод / URL:** `POST {DashboardUrl}/api/v1/agent/storages`
- **Клиент:** `Services/Dashboard/Sync/StorageSyncService`
- **Retry:** Polly, 3 попытки (1/2/4 с)
- **Ожидаемые ответы:** `204 No Content`, `400 Bad Request`, `401 Unauthorized`
