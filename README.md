...
- **Дамп и manifest GC не трогает** — подчищается только пул `chunks/`. Удаление дамп-папок по retention — см. следующий раздел.
...
2. Для каждой записи резолвит хранилище по имени из конфига `Storages[]` и удаляет `dump.*.enc` и (если был файловый бэкап) `manifest.json.enc`.
...
- **Только S3.** SFTP-записи в этой итерации пропускаются с debug-логом — у `SftpUploadService` ещё нет операции `Delete`.
...
### Очистка устаревших бэкапов (опционально)
