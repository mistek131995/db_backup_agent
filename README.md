...
```
Dump → Encrypt → Upload → Cleanup → File Backup (S3 / Azure Blob / LocalFs) → Report
```
...
**Работает на S3, Azure Blob и LocalFs.** На SFTP- и WebDAV-хранилищах file-set не запускается — в дашборде запись будет со статусом `failed` и понятным сообщением.
...
- **Restore** — скачивает дамп, расшифровывает, пересоздаёт target БД и восстанавливает её, а при наличии файлового манифеста — восстанавливает и файлы. Падение отдельного файла не валит задачу (статус `partial`). Поддерживаются все провайдеры; на SFTP и WebDAV манифеста физически нет (file-этап там не запускался), поэтому файловая часть restore автоматически пропускается. Полный разбор — в [docs/restore.md](docs/restore.md).
...
- **Файловые наборы:** произвольные каталоги с рекурсивным обходом (S3, Azure Blob, LocalFs).
- **Хранилища:** S3-совместимые (MinIO, Yandex Object Storage, AWS S3, Cloudflare R2), Azure Blob и LocalFs (локальная папка / смонтированная NFS/CIFS-шара) — полный функционал, включая дедуплицированный файловый бэкап, file-set'ы и chunk GC; SFTP и WebDAV (Яндекс.Диск, Nextcloud, ownCloud) — upload/download/delete дампа БД (включая restore через дашборд), без chunk GC, файлового бэкапа и file-set'ов.
